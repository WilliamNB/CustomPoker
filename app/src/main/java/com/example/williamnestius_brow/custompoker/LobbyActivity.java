...
public class LobbyActivity extends AppCompatActivity {
    ...
    private int maxBuyIn, minBuyIn, bigBlindValue, smallBlindValue, minBet;
        ...
        myRefG.child(gameID).child("isActive").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (active == true) {
                    ...
                    Log.d("autoload", "true");
                    String amountInput = startAmount.getText().toString();
                    if (amountInput.isEmpty()) { ;
                        int StartAmount = minBuyIn;
                        if(isHost){
                            myRefG.child(gameID).child("host").child("currentAmount").setValue(StartAmount);
                        }else {
                            myRefG.child(gameID).child("player" + sPlayerId).child("currentAmount").setValue(StartAmount);
                        }
                    } else {
                        int StartAmount = Integer.parseInt(startAmount.getText().toString());
                        if(StartAmount > maxBuyIn){
                            if (isHost) {
                                myRefG.child(gameID).child("host").child("currentAmount").setValue(maxBuyIn);
                            } else {
                                myRefG.child(gameID).child("player" + sPlayerId).child("currentAmount").setValue(maxBuyIn);
                            }
                        }else if(StartAmount < minBuyIn){
                            if (isHost) {
                                myRefG.child(gameID).child("host").child("currentAmount").setValue(minBuyIn);
                            } else {
                                myRefG.child(gameID).child("player" + sPlayerId).child("currentAmount").setValue(minBuyIn);
                            }
                        }else {
                            if (isHost) {
                                myRefG.child(gameID).child("host").child("currentAmount").setValue(StartAmount);
    ...
    public void getRules(){
        myRefR.child("Cash").child(ruleID).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                maxBuyIn =  snapshot.child("maxBuyIn").getValue(int.class);
                minBuyIn = snapshot.child("minBuyIn").getValue(int.class);
                ...
                String enterHint = "Min £" + minBuyIn + "   Max £" + maxBuyIn;
                EditText SetHint = (EditText) findViewById(R.id.EtStartAmount);
                SetHint.setHint(enterHint);
            }
            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
            }
    ...
    private void enterGame(){
        ...
        Intent startIntent = new Intent(this, CashGameActivity.class);
        startIntent.putExtra("GameID", gameID);
        startIntent.putExtra("RuleID", ruleID);
        startIntent.putExtra("IsHost", isHost);
        startIntent.putExtra("MaxBuyIn", maxBuyIn);
        startIntent.putExtra("MinBuyIn", minBuyIn);
